...
import static in.jvapps.system_alert_window.utils.Constants.WIDGET_STYLE;
import static in.jvapps.system_alert_window.utils.Constants.KEY_IMG_BTN;
...
public class BodyView {
    ...
    private View createColumn(Map<String, Object> columnMap){
        ...
        int widgetStyle = (int)columnMap.get(WIDGET_STYLE);
        View subView = null;
        if (widgetStyle == 0) {
            subView = UiBuilder.getTextView(context, Commons.getMapFromObject(columnMap, KEY_TEXT));
        }else if (widgetStyle == 1) {
            subView = UiBuilder.getImgButton(context, Commons.getMapFromObject(columnMap, KEY_IMG_BTN));
        }
        columnLayout.addView(subView);
        return columnLayout;
    }
}
